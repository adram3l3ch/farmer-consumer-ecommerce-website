...
const createProduct = async (req, res) => {
	...
	if (user.category === "Consumer")
		throw new AuthenticationError("You need a farmer account to do this action");
...
const getAllProducts = async (req, res) => {
	let products;
	if (req.params.userId) products = await Product.find({ createdBy: req.params.userId });
...
const reviewProduct = async (req, res) => {
	...
	const product = await Product.findByIdAndUpdate(
		id,
		{
			$push: { reviews: { ...req.body, createdBy: { id: req.user.id, name: req.user.name } } },
		},
		{ new: true }
	);
	res.json(product);
