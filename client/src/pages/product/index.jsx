...
const Product = () => {
	...
	useEffect(() => {
		const getProductData = async () => {
			...
			const { data: seller } = await axiosConfig.get("/users/" + product.createdBy);
			setProduct(product);
			setSeller(seller);
		};
		getProductData();
	...
	const averageReview = (
		product.reviews?.reduce((acc, val) => acc + +val.count, 0) / product.reviews?.length || 0
	).toFixed(1);
	const shouldDisplayReview = !isOwnProduct && product.reviews?.every(val => val.createdBy.id !== userId);

	const submitReview = async formData => {
		if (formData.review && review) {
			const { data } = await axiosConfig.patch("/products/review/" + product._id, {
				count: review,
		...
		<div className="productDetails">
			...
			<div className="product__right">
				...
				<h2 className="product__price">
					&#8377;{product.price} <span>({product.stock} KG Left)</span>
				</h2>
				...
				<div className="sellerDetails">
					<p>Seller Details</p>
					<p className="seller__name">{seller.name}</p>
					<address>
						{seller.phone} <br />
						{seller.address} <br />
						{seller.district}
